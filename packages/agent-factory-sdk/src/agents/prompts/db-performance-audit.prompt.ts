...
- Run the audit as explicit tasks and keep task progress visible with todo tools.
...
Run these phases in order and track with todo tools:
...
1. todowrite — write all task phases above
2. detect_db_engine — engine version, capabilities, pg_stat_statements availability
3. get_infra_runtime_signals — sessions, waits, IO, checkpoints, key settings
4. get_recent_db_logs — log signals; report limitations if access fails
5. get_statistics_health — stale stats, never-analyzed tables, pg_stat_statements reset time
6. get_lock_and_blocking_analysis — blocking chains, idle-in-transaction, long-running queries
7. get_bloat_estimates — table and index bloat quantification
8. get_replication_health — streaming lag, slot status (skip narrative section if hasReplication=false)
9. get_top_slow_queries - workload profile; use sourceNotes for reset-time caveats
10. explain_query_plan on priority queries
...
11. get_index_health - seq scan pressure, unused indexes, duplicates
12. get_table_health - dead tuples, modSinceAnalyze, temporal vacuum timestamps, autovacuum overrides
13. For each solution that will appear in the final report:
...
15. Synthesize and present the full report
