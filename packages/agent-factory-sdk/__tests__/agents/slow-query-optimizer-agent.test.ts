...
describe('SlowQueryOptimizerAgent', () => {
  ...
  it('instructs the agent to validate only evidence-backed query fixes', () => {
    const requiredPhrases = [
      ...
      'Always inspect the full execution plan for the chosen hotspot with `explain_query_plan`',
      ...
      'Configuration tuning belongs to the DB audit agent, not this optimizer.',
      'Blocked - no validated GFS remediation for this query.',
      'Choose representative validation literals from observed data distribution rather than arbitrary convenient values.',
      'Optimization incomplete: not all candidate fixes could be executed in GFS.',
